...
# Simulate the physics manually to debug
pit_loss = 32.4
outlap_delta = 0.5
gap_seconds = 7.3
degradation_penalty = 0.14

print("Manual undercut simulation:")
...
stint_length = 4  # typical undercut window
...
for lap in range(stint_length):
    # Driver A gains time on fresh tires
    initial_advantage = 3.5  # seconds per lap
    tire_advantage = initial_advantage * (0.8 ** lap)
    cumulative_driver_a_advantage += tire_advantage
    
    # Driver B loses time due to degradation
    lap_degradation = degradation_penalty * (1.2 ** lap)
