...
const GRAND_PRIX_OPTIONS = [
  ...
  { value: "spain", label: "🇪🇸 Circuit de Barcelona-Catalunya" },
  { value: "canada", label: "🇨🇦 Circuit Gilles Villeneuve" },
  { value: "austria", label: "🇦🇹 Red Bull Ring" },
  { value: "silverstone", label: "🇬🇧 Silverstone Circuit" },
  { value: "hungary", label: "🇭🇺 Hungaroring" },
  { value: "belgium", label: "🇧🇪 Circuit de Spa-Francorchamps" },
  { value: "netherlands", label: "🇳🇱 Circuit Zandvoort" },
  { value: "singapore", label: "🇸🇬 Marina Bay Street Circuit" },
  { value: "japan", label: "🇯🇵 Suzuka International Racing Course" },
  { value: "qatar", label: "🇶🇦 Lusail International Circuit" },
  { value: "usa", label: "🇺🇸 Circuit of the Americas" },
  { value: "mexico", label: "🇲🇽 Autódromo Hermanos Rodríguez" },
  { value: "brazil", label: "🇧🇷 Autódromo José Carlos Pace" },
  { value: "las_vegas", label: "🇺🇸 Las Vegas Street Circuit" },
  { value: "abu_dhabi", label: "🇦🇪 Yas Marina Circuit" },
  { value: "australia", label: "🇦🇺 Albert Park Circuit" },
  { value: "china", label: "🇨🇳 Shanghai International Circuit" },
  { value: "azerbaijan", label: "🇦🇿 Baku City Circuit" },
  { value: "miami", label: "🇺🇸 Miami International Autodrome" },
  { value: "saudi_arabia", label: "🇸🇦 Jeddah Corniche Circuit" },
];
...
const COMPOUND_OPTIONS = [
  {
    value: "SOFT",
    label: "🔴 Soft Compound",
    description: "Fastest degradation, maximum grip",
  },
  {
    value: "MEDIUM",
  ...
  },
  {
    value: "HARD",
    label: "⚪ Hard Compound",
    description: "Longest lasting, slowest lap times",
  },
];
...
export const SimulationForm: React.FC<SimulationFormProps> = ({
  ...
  const simulationMutation = useMutation({
    mutationFn: async (data: SimulationFormData) => {
      const request: SimulationRequest = {
        ...data,
        samples: data.samples || 1000, // Provide default value
    ...
    onError: (error) => {
      addToast({
        ...
        description:
          error.message || "An unexpected error occurred during simulation",
        variant: "destructive",
        duration: 8000,
      });
    },
  ...
  return (
    ...
    >
        ...
        {simulationMutation.isPending ? (
          <>
            ...
            <span className="sr-only">
              Please wait, Monte Carlo simulation in progress
            </span>
          </>
        ) : (
          <>
      ...
      >
        Executes Monte Carlo simulation to calculate undercut success
        probability
      </p>
    </form>
  );
