...
const prepareHeatmapData = (data: HeatmapDataPoint[]) => {
    ...
    const gaps = Array.from(new Set(data.map(d => d.gap))).sort((a, b) => a - b);
...
const EmptyState: React.FC = () => (
    <div className="text-center py-12" role="status" aria-live="polite">
        ...
        <p className="text-gray-500 dark:text-gray-400 max-w-sm mx-auto mb-4">
            Run a simulation first to generate the heatmap analysis across different gaps and compounds.
        </p>
        <p className="text-sm text-gray-400 dark:text-gray-500">
            The heatmap will show undercut success probability for various gap sizes and tire compounds.
        </p>
    </div>
);

const LoadingState: React.FC = () => (
    <div className="space-y-4" role="status" aria-live="polite" aria-label="Generating heatmap analysis">
        ...
        <p className="text-center text-sm text-gray-500 dark:text-gray-400">
            Running simulations across {Math.ceil((GAP_RANGE.max - GAP_RANGE.min) / GAP_RANGE.step) + 1} gaps × 3 compounds...
            <br />
...
export const Heatmap: React.FC<HeatmapProps> = ({ currentRequest }) => {
    ...
    const {
        ...
        queryFn: () => baseRequest ? apiClient.generateHeatmapData(baseRequest, GAP_RANGE) : null,
        enabled: !!baseRequest,
            ...
            <Alert role="status" aria-live="polite">
                ...
                <AlertDescription>
                    <div className="space-y-2">
                        ...
                        <p>The simulation completed but no data was returned. This might happen if:</p>
                        <ul className="list-disc list-inside text-sm space-y-1">
                            <li>Network connectivity issues occurred during data generation</li>
    ...
    return (
        <div className="space-y-6">
            ...
            <Alert>
                <Info className="h-4 w-4" aria-hidden="true" />
                <AlertDescription>
                    <strong>Heatmap Analysis:</strong> This visualization shows undercut success probability across different gap sizes (0-30s) and tire compounds.
                    Darker/warmer colors indicate higher success probability. Hover over cells for detailed values.
                </AlertDescription>
            </Alert>
            ...
            <Card>
                <CardContent className="p-6">
                    <div
                        role="img"
                        aria-label={`Heatmap showing undercut success probability for ${baseRequest.gp.toUpperCase()} ${baseRequest.year}. Interactive chart with gap sizes on X-axis and tire compounds on Y-axis.`}
                    >
                        <Plot
                            data={[
                                {
                            ...
                            layout={{
                                title: {
                                    text: `Undercut Success Probability - ${baseRequest.gp.toUpperCase()} ${baseRequest.year}`,
                                    font: { size: 16 }
                                },
                                xaxis: {
                ...
                <Card>
                    <CardContent className="p-4">
                        <div
                            ...
                            role="img"
                            aria-label={`High success scenarios: ${plotData.z.flat().filter(v => v >= 50).length} out of ${plotData.z.flat().length} total scenarios`}
                        >
                            <p className="text-2xl font-bold text-green-600 dark:text-green-400">
                                {plotData.z.flat().filter(v => v >= 50).length}
                            </p>
                ...
                <Card>
                    <CardContent className="p-4">
                        <div
                            className="text-center"
                            role="img"
                            aria-label={`Average success rate: ${Math.round(plotData.z.flat().reduce((a, b) => a + b, 0) / plotData.z.flat().length)}%`}
                        >
                            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                                {Math.round(plotData.z.flat().reduce((a, b) => a + b, 0) / plotData.z.flat().length)}%
                            </p>
                            <p className="text-sm font-medium text-gray-900 dark:text-white">
                ...
                <Card>
                    <CardContent className="p-4">
                        <div
                            className="text-center"
                            role="img"
                            aria-label={`Best compound: ${plotData.compounds.find(c => {
                                const compoundIndex = plotData.compounds.indexOf(c);
                                const avgForCompound = plotData.z[compoundIndex].reduce((a, b) => a + b, 0) / plotData.z[compoundIndex].length;
                                return avgForCompound === Math.max(...plotData.compounds.map(comp => {
                                    const idx = plotData.compounds.indexOf(comp);
                                    return plotData.z[idx].reduce((a, b) => a + b, 0) / plotData.z[idx].length;
                                }));
                            }) || 'N/A'} compound has highest average success rate`}
                        >
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                ...
                <ul className="space-y-1">
                    ...
                    <li>• Summary statistics are provided in text format below the chart</li>
                </ul>
            </div>
        </div>
    );
