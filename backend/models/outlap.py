...
class OutlapModel:
    ...
    def fit(self, df_laps: pd.DataFrame) -> "OutlapModel":
        ...
        lap_time_col = self._find_column(
            df_laps, ["lap_time", "lap_duration", "laptime"]
        )
        ...
        df_work[compound_col] = df_work[compound_col].replace(
    ...
    def sample(
        self, compound: str, n: int = 1, rng: np.random.Generator | None = None
    ) -> float | np.ndarray:
        """
