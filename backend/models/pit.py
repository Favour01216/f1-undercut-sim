...
class PitModel:
    ...
    def __init__(self):
        ...
        self.std_loss: float | None = None
        self.fitted: bool = False
        self.sample_count: int = 0
        self.distribution: stats.norm | None = None
    ...
    def fit(self, df_pit_events: pd.DataFrame) -> "PitModel":
        ...
        if not pit_duration_col and not time_loss_col:
            raise ValueError(
                "No pit time column found. Expected 'pit_duration', 'duration', 'time_loss', or 'pit_loss_time'"
            )
        ...
        if len(time_losses) < 3:
            raise ValueError(
                f"Insufficient data for fitting. Need at least 3 valid points, got {len(time_losses)}"
            )
        ...
        time_losses_clean = time_losses_clean[
            (time_losses_clean >= 10) & (time_losses_clean <= 60)
        ]
        ...
        logger.info(
            f"Pit model fitted: μ={self.mean_loss:.2f}s, σ={self.std_loss:.2f}s ({self.sample_count} samples)"
        )
    ...
    def get_percentiles(self, percentiles: list | None = None) -> dict[int, float]:
        """
    ...
    def compare_scenarios(self, scenarios: dict[str, float]) -> dict[str, Any]:
        ...
        for scenario_name, target_time in scenarios.items():
            ...
            results[scenario_name] = {
                ...
                "expected_advantage": (
                    float(self.mean_loss - target_time)
                    if target_time < self.mean_loss
                    else 0
                ),
                "risk_level": (
                    "low"
                    if probability > 0.7
                    else "medium" if probability > 0.3 else "high"
                ),
            }
