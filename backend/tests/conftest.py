...
def prevent_network_calls():
    ...
    def raise_network_error(*args, **kwargs):
        raise RuntimeError(
            "Network calls are not allowed in unit tests. Use @pytest.mark.integration for tests that need real API calls."
        )

    # Patch common HTTP libraries
    with (
        patch("requests.get", side_effect=raise_network_error),
...
def mock_api_calls():
    ...
    with (
        patch("backend.services.openf1.OpenF1Client.get_laps") as mock_get_laps,
        patch(
            "backend.services.openf1.OpenF1Client.get_pit_events"
        ) as mock_get_pit_events,
        patch("backend.services.jolpica.JolpicaClient.get_results") as mock_get_results,
        patch(
            "backend.services.jolpica.JolpicaClient.get_schedule"
