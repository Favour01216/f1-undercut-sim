...
class TestPitModel:
    ...
    def test_fit_with_pit_duration_only(self):
        ...
        model = PitModel()
        df = pd.DataFrame({
            'pit_duration': [20.0, 22.5, 21.0, 23.8, 19.5]
        })
        
        model.fit(df)
        assert model.fitted
        assert model.mean_loss > 20  # Should be higher than pit duration due to overhead
    
    def test_alternative_column_names(self):
        """Test fitting with alternative column names."""
        model = PitModel()
        df = pd.DataFrame({
            'duration': [20.0, 22.5, 21.0, 23.8, 19.5]
        })
    ...
    def test_outlier_handling(self):
        ...
        df = pd.DataFrame({
            'time_loss': normal_times + outlier_times
        })
    ...
    def test_minimum_variability(self):
        ...
        df = pd.DataFrame({
            'time_loss': [25.0, 25.0, 25.0, 25.0, 25.0]
        })
